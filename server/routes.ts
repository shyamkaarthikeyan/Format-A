          ...
          pdfPython.on('close', async (pdfCode: number) => {
            ...
            try {
              if (pdfCode !== 0) {
                ...
                // Return error instead of using fallback (which had comma-separated author format)
                return res.status(500).json({ 
                  ...
                  workingDirectory: __dirname,
                  suggestion: 'PDF generation may not be available on this hosting platform. Please try downloading the DOCX file instead.'
                });
              }
